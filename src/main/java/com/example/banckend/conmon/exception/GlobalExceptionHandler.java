...
import com.example.banckend.conmon.response.ErrorResponse;
...
public class GlobalExceptionHandler {
        ...
        // Thêm vào GlobalExceptionHandler.java, sau handler của CustomException

        @ExceptionHandler(BadRequestException.class)
        public ResponseEntity<ErrorResponse> handleBadRequestException(BadRequestException ex) {
                log.error("Bad Request Exception: {}", ex.getMessage(), ex);

                // Secret sai hoặc Unauthorized → 401 để ESP32 tự clearConfig() và restart
                if (ex.getStatus() == HttpStatus.UNAUTHORIZED ||
                    (ex.getMessage() != null && ex.getMessage().contains("Invalid device secret"))) {
                        return buildErrorResponse(
                                        String.valueOf(ex.getCode()),
